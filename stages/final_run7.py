...
NUMBER_SOUND_CANDIDATES = {
    1: ("numbers/one.wav", "numbers/One.wav", "one.wav", "One.wav"),
    2: ("numbers/two.wav", "numbers/Two.wav", "two.wav", "Two.wav"),
    3: ("numbers/three.wav", "numbers/Three.wav", "three.wav", "Three.wav"),
    4: ("numbers/four.wav", "numbers/Four.wav", "four.wav", "Four.wav"),
}
GOOD_JOB_SOUND_CANDIDATES = (
    "communication/good_job.wav",
    "communication/Good Job.wav",
    "communication/good-job.wav",
    "good_job.wav",
    "Good Job.wav",
    "good-job.wav",
...
class Runner(object):
    ...
    def _sound_candidates(self, rel_paths):
    ...
    def _play_existing_wav(self, rel_paths, label):
        tried = []
        for path in self._sound_candidates(rel_paths):
            tried.append(path)
            try:
            ...
            if not exists:
                continue
            if hasattr(self.hw, "_sound") and getattr(self.hw, "_sound", None) is None:
                continue
            ...
            if callable(player):
                ...
                return True
            player = getattr(self.hw, "play_file", None)
            if callable(player) and player(path):
                self.log.log("AUDIO", "WAV_" + label, path=path)
                return True
        self.log.log("AUDIO", "WAV_MISSING_" + label,
                     tried=tried[:8], tried_count=len(tried))
        return False
    ...
    def diagnose_audio_once(self):
        ...
        for root in SOUND_ROOTS:
            try:
                ...
                pass
        sound_ready = getattr(self.hw, "_sound", None) is not None
        good_job_found = False
        for path in self._sound_candidates(GOOD_JOB_SOUND_CANDIDATES):
            try:
                good_job_found = os.path.exists(path)
            except Exception:
                good_job_found = False
            if good_job_found:
                break
        self.log.log("AUDIO_DIAG", "EV3_SOUND_CHECK",
                     sound_ready=sound_ready, roots=";".join(roots),
                     good_job_found=good_job_found)
        if (not sound_ready) or (not roots) or (not good_job_found):
            self.screen(("AUDIO DIAG",
                         "wav {}".format("OK" if good_job_found else "MISS"),
                         "fallback ready"))
            time.sleep(0.4)

    def say_number(self, number):
        candidates = NUMBER_SOUND_CANDIDATES.get(int(number), ())
        if candidates and self._play_existing_wav(candidates, "NUMBER"):
            return
        speaker = getattr(self.hw, "say_number", None)
        if callable(speaker) and speaker(number):
            self.log.log("AUDIO", "NUMBER_FALLBACK_SAY", number=number)
            return
        speaker = getattr(self.hw, "speak", None)
        if callable(speaker) and speaker(str(number)):
            self.log.log("AUDIO", "NUMBER_FALLBACK_SPEAK", number=number)
            return
        self.play(((500 + int(number) * 80, 250),))
        self.log.log("AUDIO", "NUMBER_FALLBACK_TONE", number=number)

    def say_good_job(self):
        if self._play_existing_wav(GOOD_JOB_SOUND_CANDIDATES, "GOOD_JOB"):
            return
        speaker = getattr(self.hw, "say_good_job", None)
        if callable(speaker) and speaker():
            self.log.log("AUDIO", "GOOD_JOB_FALLBACK_SAY")
            return
        speaker = getattr(self.hw, "speak", None)
        if callable(speaker) and speaker("Good job"):
            self.log.log("AUDIO", "GOOD_JOB_FALLBACK_SPEAK")
            return
        self.play(((900, 120), (1200, 160), (900, 120)))
        self.log.log("AUDIO", "GOOD_JOB_FALLBACK_TONE")

    def say_blue(self, number):
        speaker = getattr(self.hw, "say_blue", None)
        if callable(speaker) and speaker(number):
            return
        words = {1: "one", 2: "two", 3: "three", 4: "four"}
        speaker = getattr(self.hw, "speak", None)
        text = "Blue " + words.get(int(number), str(number))
        if callable(speaker) and speaker(text):
            return
