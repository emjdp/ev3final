#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gg5 — gg3 + 고속 적응 주행층(판단/미션/UI 로직은 gg3 그대로).

고속 적응 주행층(gg5 신설 — "빨라도 안정" 4종 세트):
  ...
  파라미터 승계: config/gg5.json 이 없으면 gg3.json 의 실기 튜닝값
  (캘리브레이션·kp·임계값 등)을 자동 승계한다 — 속도 계열(base_speed/
  turn_speed)만 gg5 기본값(고속)을 쓴다. 트랙이 바뀌었으면 calibrate 1회.
...
실행(브릭):   python3 stages/gg5.py
문법 점검(PC): python3 -m py_compile stages/gg5.py lib/*.py
단위 테스트:  python3 tests/test_gg5_logic.py (ev3dev2 불필요)
"""
...
SAVE_PATH = os.path.join(_ROOT, "config", "gg5.json")
STAGE_NAME = "gg5"

# gg3 실기 튜닝값 승계(최초 1회) — gg5.json 이 없으면 gg3.json 에서
# 캘리브레이션·조향·임계값을 가져온다. 속도 계열은 gg5 기본값(고속) 유지.
GG3_SAVE_PATH = os.path.join(_ROOT, "config", "gg3.json")
...
def seed_initial_from_gg3(initial):
    """gg5 첫 실행 승계: gg5.json 이 없으면 gg3.json 의 실기 튜닝값을
    initial(INITIAL_PARAMS 사본)에 병합해 반환한다(승계된 이름 리스트).

    SharedParams.set 은 max_step 제한이 있어 생성 '전'의 기본값 단계에서
    병합한다. 속도 계열(SEED_SKIP)은 gg5 기본값(고속)을 유지하고, 값은
    gg5 한계로 클램프한다. gg5.json 이 이미 있으면 아무 것도 하지 않는다
    (load_saved_into_defaults 가 저장값으로 덮는다)."""
    if os.path.exists(SAVE_PATH) or not os.path.exists(GG3_SAVE_PATH):
        return []
    try:
        with open(GG3_SAVE_PATH, "r") as fp:
            saved = json.load(fp)
    except Exception:
        return []
...
def run():
    ...
    seeded = seed_initial_from_gg3(initial)
    ...
    if seeded:
        log.log("PARAM_SEED", "GG3_CARRYOVER", names=",".join(seeded))
    print("gg5 ready. 1) dashboard 'calibrate' on the line, 2) place robot "
    ...
    try:
        ...
        try:
            ...
            server.stop()
    print("gg5 stopped. sessions={}".format(runner.session))
