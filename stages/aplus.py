...
역할 분담:
  로봇(자동) — gg5/fxck2 실기 검증분 그대로:
    ...
    - UI(fxck3 승계, 전부 비동기): 버튼 출발+스톱워치, 랜덤 숫자(1~4),
  ...
  사람(조종 패드) — 애매한 '결정' 전부:
    - 결정 지점 = 분기(출구 2+)/막힘/유실 확정/빨강/초록(배달 후) —
      ...
      서서 사람을 부른다(꼬임 제거 — q/e/s 로 사람이 푼다).
    - 명령 키(조종 패드 = 액션 manifest key):
      ...
      q/e 는 위치 교정용 아크 전진 — 실행 후 그 자리에서 계속 대기한다
      (연타로 미세 조정 후 [w] 로 재개). w/a/s/d 는 이동 후 라인추종 재개.
...
마커 미션 분기(fxck3 승계 — 이동 판단은 전부 사람):
  빨강: "red N" 재생 후 결정 대기(보통 [s] 유턴).
  초록(최초): good_job + OUT 시간 + 랜덤 숫자 갱신·재생 + 배달(전진→그립
...
# DL/DR(대각선)은 실행 후에도 그 자리에서 계속 대기(위치 교정).
...
ACTIONS = [
    ...
    {"name": "uturn", "label": "CMD U-Turn 180", "key": "s"},
...
class Runner(object):
    ...
    def turn(self, move):
        ...
        [a]/[d]=90도, [s]=180도 유턴. "S" 는 no-op(그대로 직진 재개)."""
        if move == "S":
            return
        snap = self.params.snapshot()
        if move == "U":
    ...
    def decide_and_move(self, kind, has_left, has_right, has_straight,
        ...
        정지 상태로 기다린다(패드가 배경색으로 알림). 대각선(DL/DR)은 실행 후 같은
        결정 지점에서 계속 명령을 소비/대기한다(위치 교정 체이닝) —
        w/a/s/d 가 오면 이동 후 라인추종으로 복귀한다. 명령은 출구 유효성
        검사 없이 실행한다(조종자가 보스) — 다르면 로그에 흔적만 남긴다."""
...
def run():
    ...
    print("aplus ready — REMOTE DECISION MODE (base 15%). robot line-follows, "
          ...
          "[s]uturn180 [q]diag-L [e]diag-R (stay) [p]grip-close [o]open "
          "[x]clear [t]GO [n]calibrate [z]reset. commands queue FIFO (max 8) "
