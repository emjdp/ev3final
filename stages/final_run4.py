#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""final_run4 — final_run3 + 버튼 시작/음성 안내/스톱워치 표시.
...
실행(브릭):   python3 stages/final_run4.py
문법 점검(PC): python3 -m py_compile stages/final_run4.py lib/*.py
"""
...
DISPLAY_REFRESH_S = 0.3     # LCD 재출력 주기 — 기본 ev3dev 메뉴가 덮지 못하게 유지
...
NUMBER_SOUNDS = {
    1: os.path.join(SOUND_ROOT, "numbers", "one.wav"),
    2: os.path.join(SOUND_ROOT, "numbers", "two.wav"),
    3: os.path.join(SOUND_ROOT, "numbers", "three.wav"),
    4: os.path.join(SOUND_ROOT, "numbers", "four.wav"),
    5: os.path.join(SOUND_ROOT, "numbers", "five.wav"),
    6: os.path.join(SOUND_ROOT, "numbers", "six.wav"),
    7: os.path.join(SOUND_ROOT, "numbers", "seven.wav"),
    8: os.path.join(SOUND_ROOT, "numbers", "eight.wav"),
    9: os.path.join(SOUND_ROOT, "numbers", "nine.wav"),
    10: os.path.join(SOUND_ROOT, "numbers", "ten.wav"),
}
...
PARAM_TABLE = (
    ...
    ("ki",              0.0,   0.0, 0.5,  0.05, 0.01, ""),   # 0 = I 제어 off(기본)
...
SAVE_PATH = os.path.join(_ROOT, "config", "final_run4.json")
STAGE_NAME = "final_run4"
...
class Explorer(object):
    ...
    final_run4: HOME 은 build_return_route 의 스텝 리스트(route)를 소비한다.
    ...
    def on_probe_end(self, kind):
        ...
        final_run4: 팔 끝 종류(kind)를 arm_ends 에 기록한다 — 복귀 계획의
...
class Runner(object):
    ...
    def __init__(self, hw, params, tele, log):
        ...
        # LCD 유지 스레드 — 이벤트로만 그리면 기본 ev3dev 메뉴가 곧 화면을 덮으므로
        # 표시할 내용이 생긴 뒤부터 현재 상태를 주기적으로 재출력한다(daemon).
    ...
    def _init_session_state(self):
        ...
        self.grabbed = False
        self.done = False
        self.out_timer_start = None
        self.return_timer_start = None
        self.out_elapsed = None
        self.return_elapsed = None
        self.bottom_number = None
        self.out_red_spoken = 0
        self.return_red_spoken = 0
        self.last_turn = 0.0
    ...
    def publish(self, mode, **extra):
        frame = {
            ...
            "out_elapsed": self.out_elapsed,
            "return_elapsed": self.return_elapsed,
            "bottom_number": self.bottom_number,
        }
        frame.update(extra)
    ...
    def refresh_final4_display(self):
        self.hw.show_final4_display(self.out_elapsed, self.return_elapsed,
                                    self.bottom_number)

    def _display_loop(self):
        """표시할 내용(시간/숫자 중 하나라도)이 생긴 뒤부터 현재 상태를 주기적으로
        재출력해 화면을 유지한다. 내용이 없으면(시작 전/새 세션) 화면을 건드리지
        않아 브릭 기본 메뉴가 그대로 보인다."""
    ...
    def finish_out_timer(self):
        if self.out_timer_start is None or self.out_elapsed is not None:
            return
        self.out_elapsed = time.monotonic() - self.out_timer_start
        self.refresh_final4_display()
        self.hw.play_wav(SOUND_GOOD_JOB)
        self.log.log("STOPWATCH_OUT", "COLOR_GREEN",
                     elapsed_s=round(self.out_elapsed, 1))

    def finish_return_timer(self):
        if self.return_timer_start is None or self.return_elapsed is not None:
            return
        self.return_elapsed = time.monotonic() - self.return_timer_start
        self.refresh_final4_display()
        self.log.log("STOPWATCH_RETURN", "COLOR_YELLOW",
                     elapsed_s=round(self.return_elapsed, 1))

    def start_return_timer(self):
        self.return_timer_start = time.monotonic()
        self.return_elapsed = None
        self.return_red_spoken = 0
        self.choose_random_number("RETURN")
    ...
    def handle_marker(self, color, context):
        ...
        if color == COL_RED:
            ...
            self.finish_out_timer()
            self.deliver()
            if self.interrupted():
                return True
            # 배달 유턴까지 마친 뒤 탐색 중단 → 복귀 계획 수립(final_run4).
            self.turn("U")
            if self.interrupted():
                return True
            self.start_return_timer()
    ...
    def wait_for_start(self):
        """브릭 가운데 버튼 대기 → 랜덤 숫자 표시/재생 → 출발선 이탈 전진.

        버튼 대기는 timeout 폴링이라 대기 중에도 매 주기 handle_pending 이 돌아
        대시보드 calibrate([1] 키)가 즉시(버튼 전에) 실행된다."""
        snap = self.params.snapshot()
        self.hw.grip_open(snap["grip_speed"], GRIP_SEC)
        self.refresh_final4_display()   # 이전 세션 기록 화면을 비운다(상태는 전부 None)
        while True:
            if self.stop_on:
                return "stop"
            if self.reset_on:
                return "reset"
            self.handle_pending()
            self.publish("waiting_button")
            # timeout=0.15 → 안 눌리면 None 을 돌려 루프 상단으로 → handle_pending 재실행.
            if self.hw.wait_center_button(lambda: self.stop_on,
                                          lambda: self.reset_on,
                                          poll_s=0.05, timeout=0.15):
                break
        self.hw.beep_ok()
        self.out_timer_start = time.monotonic()
        self.out_elapsed = None
        self.out_red_spoken = 0
        self.choose_random_number("OUT")
    ...
    def idle_after_done(self):
        """완주 후 대기 — 그 자리 노랑에서 무한 재출발하지 않도록 대기한다.
        기록 화면(OUT/BACK/숫자)은 가운데 버튼을 누를 때까지 계속 떠 있고,
        버튼을 누르면(또는 대시보드 reset) 새 세션 대기로 돌아간다."""
        self.hw.stop()
        while True:
            if self.stop_on:
                self.log.log("EMERGENCY_STOP", "NETWORK", source=self.stop_source)
            ...
            # 가운데 버튼 = 확인 → 새 세션 대기(화면은 그때까지 유지). timeout 폴링이라
            # 대기 중에도 handle_pending/stop/reset 이 계속 반응한다.
            if self.hw.wait_center_button(lambda: self.stop_on,
                                          lambda: self.reset_on,
                                          poll_s=0.05, timeout=0.15):
                self.log.log("IDLE_ACK", "CENTER_BUTTON")
                return "reset"
...
def run():
    ...
    print("final_run4 ready. put robot ON the line and run dashboard 'calibrate' "
          "first, then press the CENTER button to start. dashboard 'reset' ([r] key) "
    ...
    try:
        ...
        try:
            ...
            server.stop()
    print("final_run4 stopped. sessions={}".format(runner.session))
