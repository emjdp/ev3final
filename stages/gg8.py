...
역할 분담:
  로봇(자동) — gg5 실기 검증분 그대로:
    ...
    - 커브(확정 후 출구 1개)는 결정 지점이 아니다 — 자동 통과(명령 소비 X)
    - 물체 자동 파지(초음파 grab_dist 이내, 그립 비었을 때)
  사람(대시보드) — 모든 '결정':
    - 결정 지점 = 분기(출구 2+)/마커(빨강·초록·노랑 확정)/막다른길.
...
명령 큐 규약(CommandBox):
  ...
  - clear 로 비울 수 있다. 현재 큐는 매 텔레메트리 프레임 pending_cmd 로
    방송된다(대시보드 상단에 표시).
  - 소비 시점: 분기 확정 후 / 마커 색 확정 후 / 막다른길 확정 후.
    커브 자동 통과와 유실 복구는 소비하지 않는다.
...
class Runner(object):
    ...
    def handle_node(self, bits, creep_mm):
        ...
        커브(출구 1개)는 결정이 아니므로 자동 통과(명령 소비 X) — gg5 동일.
        출구 2개 이상이면 대시보드 명령을 소비/대기한다."""
        snap = self.params.snapshot()
...
def run():
    ...
    print("gg8 ready — MANUAL DECISION MODE. dashboard keys: [1]L [2]S [3]R "
          ...
          "queue a move anytime; robot consumes it at junctions/markers, "
          "or stops and beeps until you send one. start: CENTER button or "
          "[0]. (Ctrl-C or robotctl stop to quit)")
    try:
        runner.run_sessions()
