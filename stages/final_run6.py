#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""final_run6 — final_run3 + 회전 시작 '틱틱' 튐 제거(가속 램프 + coast).

final_run6 변경(주행 하드웨어 계층만, 판단/복귀 로직은 final_run3 그대로):
...
실행(브릭):   python3 stages/final_run6.py
문법 점검(PC): python3 -m py_compile stages/final_run6.py lib/*.py
"""
...
PARAM_TABLE = (
    ...
    ("turn_ramp_ms",    250,   0,   600,  100,  50,   "ms"),  # 회전 가속 램프(시작 틱틱 튐 방지, final_run6)
...
SAVE_PATH = os.path.join(_ROOT, "config", "final_run6.json")
STAGE_NAME = "final_run6"
...
class Runner(object):
    ...
    def turn(self, move):
        ...
        final_run6: 회전 시작 '틱틱' 튐 제거 — (1) reset_encoders() 전에 coast()
        ...
        self.hw.coast()                 # 직전 brake-hold 해제 → 리셋 킥 방지(final_run6)
        self.hw.reset_encoders()
        self.hw.set_ramp(snap["turn_ramp_ms"])   # 가속만 램프(감속=0), final_run6
        try:
            ...
            self.hw.stop()
            self.hw.set_ramp(0)         # 라인추종 조향엔 램프 미적용(final_run6)
...
def run():
    ...
    print("final_run6 ready. put robot ON the line and run dashboard 'calibrate' "
    ...
    try:
        ...
        try:
            ...
            server.stop()
    print("final_run6 stopped. sessions={}".format(runner.session))
