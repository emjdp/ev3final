#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""final_run9 — 3반사광 PID + 중앙 RGB 색판정 + 미션 오디오/LCD (inchul_last_dance.md).
...
미션(inchul_last_dance.md 프로젝트 상세):
  ...
  2) 초록점까지 미로 탐색(좌>우>직, 분기 정리형 — return_revisit_all_minimal_route.html
     ...
     (tests/test_final_run9_maze.py 로 PC 검증).
...
final_run8 대비 변경:
  ...
  B) PID 에러를 3센서 가중 위치로 변경: error = 100*(darkL-darkR)/(darkL+darkC+darkR).
     중앙 밝기가 분모에 들어가 커브/노드 진입에서 에러가 과장되지 않고, 정중앙
     기준(=0)은 run8 의 normR-normL 과 같은 부호/스케일이라 kp/ki/deadband 재사용.
...
실행(브릭):   python3 stages/final_run9.py
문법 점검(PC): python3 -m py_compile stages/final_run9.py lib/*.py
단위 테스트:  python3 tests/test_final_run9_maze.py (ev3dev2 불필요)
...
PID_DENOM_MIN = 100.0       # 3센서 가중 위치 분모 하한(§변경 B) — 정중앙에서
                            # darkC(~100) 하나만 분모일 때 run8 스케일과 일치
...
PARAM_TABLE = (
    ...
    ("deadband",        2.0,   0.0, 20.0, 2.0,  0.5,  ""),   # 3.0→2.0 살짝 타이트(더 작은 오차에 조향)
...
SAVE_PATH = os.path.join(_ROOT, "config", "final_run9.json")
STAGE_NAME = "final_run9"
...
class PidSteer(object):
    """error = 3센서 가중 위치(§변경 B): 100*(darkL-darkR)/(darkL+darkC+darkR).

    dark = 100-norm(어두움). 정중앙(중앙만 검정)에선 분모가 darkC 뿐이라
    run8 의 normR-normL 과 같은 값·부호가 되고, 커브/노드 진입처럼 여러
    센서가 검정을 밟는 순간엔 분모가 커져 에러가 과장되지 않는다. 전백
    (000 유실 의심)은 분모 하한(PID_DENOM_MIN)으로 0 에 수렴.
    ...
    def step(self, norm_l, norm_c, norm_r, snap, base_speed):
        dark_l = 100.0 - float(norm_l)
        dark_c = 100.0 - float(norm_c)
        dark_r = 100.0 - float(norm_r)
        denom = max(dark_l + dark_c + dark_r, PID_DENOM_MIN)
        error = 100.0 * (dark_l - dark_r) / denom
...
class Runner(object):
    ...
    def turn(self, move):
        ...
        if not self.interrupted():
            snap2 = self.params.snapshot()
            color, norm_c = self.read_color(snap2)
            if (norm_c >= snap2["center_th_node"] and
                    color not in MARKER_COLORS):
                self.log.log("TURN_ACQUIRE", "CENTER_OFF_LINE_AFTER_TURN",
                             move=move, color=color, norm_c=round(norm_c, 1))
                self.realign_to_line(snap2)
...
def run():
    ...
    runner.ensure_sounds()      # espeak wav 캐시 생성(최초 1회만 느리다)

    print("final_run9 ready. 1) dashboard 'calibrate' on the line / 'read_rgb' on "
    ...
    try:
        ...
        try:
            ...
            server.stop()
    print("final_run9 stopped. sessions={}".format(runner.session))
