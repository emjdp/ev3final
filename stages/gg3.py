#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gg3 — gg2 + 소리·LCD·버튼 UI 층(주행/판단 로직은 gg2 그대로).
...
실행(브릭):   python3 stages/gg3.py
문법 점검(PC): python3 -m py_compile stages/gg3.py lib/*.py
단위 테스트:  python3 tests/test_gg3_logic.py (ev3dev2 불필요)
"""
...
SAVE_PATH = os.path.join(_ROOT, "config", "gg3.json")
STAGE_NAME = "gg3"
...
def run():
    ...
    print("gg3 ready. 1) dashboard 'calibrate' on the line, 2) place robot "
    ...
    try:
        ...
        try:
            ...
            server.stop()
    print("gg3 stopped. sessions={}".format(runner.session))
