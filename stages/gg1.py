#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gg1 — 왕복 운반: 안정 주행(라인추종/노드/유실) + 전노드 재방문 복귀.
...
규약: Python 3.5(f-string 금지) / ev3dev2 는 run() 안에서만 import /
      정지는 네트워크 stop 또는 Ctrl-C, 재시작은 네트워크 reset. 소리 없음.

실행(브릭):   python3 stages/gg1.py
문법 점검(PC): python3 -m py_compile stages/gg1.py lib/*.py
단위 테스트:  python3 tests/test_gg1_logic.py (ev3dev2 불필요)
"""
...
SAVE_PATH = os.path.join(_ROOT, "config", "gg1.json")
STAGE_NAME = "gg1"
...
class Explorer(object):
    ...
    def on_arrive_home(self, kind, has_left=False, has_right=False,
        ...
        계산해 반환한다(리프에선 자연히 유턴). 불일치·move 불가·계획 소진이면
        RETURN_FALLBACK 후 즉석 탐색(좌>우>직) — 절대 멈추지 않는다.
...
def run():
    ...
    print("gg1 ready (silent). 1) dashboard 'calibrate' on the line, "
          "2) place robot on YELLOW to start. it grips the object when it "
    ...
    try:
        ...
        try:
            ...
            server.stop()
    print("gg1 stopped. sessions={}".format(runner.session))
