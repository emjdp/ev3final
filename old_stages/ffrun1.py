#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ffrun1 — final_run11 + 이벤트 유실 수정 + 색상 가이드 캘리브레이션.

final_run11 대비 변경 — 주행/판단층은 그대로, 대시보드 연동만 바뀐다.

  J) 노랑 → 파랑 교체(2026-07-10, 사용자 — 실기 센서가 노랑을 끝내 인식하지
     못해 코스의 노랑 스티커를 전부 파랑으로 교체): 출발지/막다른 표식 마커가
     COL_BLUE 가 되고, 위저드 순서도 WHITE → BLACK → RED → GREEN → BLUE.
     아래 원문 헤더들의 '노랑/노란점'은 전부 '파랑/파란점'으로 읽어야 한다.
     **실기 판정축(같은 날 실측)**: 이 센서는 파란 스티커를 청록처럼 읽는다
     — r=40 g=146 b=119, 즉 g 가 b 보다 강하다(b/g≈0.82). 처음 설계했던
     b 채널 지배(min(b/r, b/g))로는 절대 통과 못 하므로, 판정은 'r 가 최약'
     축(min(b/r, g/r) > rgb_blue_ratio — 실측 2.98 vs 흰 바닥 0.79)으로 한다.
     g/r 축을 초록과 공유하므로 classify 는 blue 를 green 보다 먼저 검사하고
     (진짜 초록은 b 최약이라 blue 를 통과 못 함), 위저드 green 차례에는
     blue 지배비 교차 검사(LOOKS_BLUE)를 둔다.

  K) 라인트레이싱 캘리 실측(2026-07-10, 실측값.md — 한 지점 5자세):
     white 밝기 89.8~91.7 / black 11.5 / 엣지 정위치 62.3(왼쪽 치우침 91.7,
     오른쪽 치우침 17.3). → cal_c_black=12, cal_c_white=91 로 정규화하면
     엣지 정위치 norm_c ≈ 64. 엣지 밝기는 검/백 산술 중간(51)이 아니라
     그보다 밝으므로 edge_target 기본값을 50 → 64 로 올린다(센서 스팟이
     라인에 반쯤만 걸치는 위치가 '정위치'이기 때문).

  H) 단발 이벤트 유실 수정(사용자 문제 — "[2] read_rgb 를 눌러도 이벤트창에
     아무것도 안 뜬다"): 브릭 Telemetry 는 '최신 프레임 1개'만 보관하는데,
     read_rgb 는 COLOR_READ 이벤트를 프레임 1개에 실어 publish 하고 곧바로
     wait_for_start 루프의 waiting_start 프레임(~130ms 간격)이 덮어쓴다.
     노트북 telemetry_watcher 는 4Hz(250ms)로 폴링하므로 그 사이 프레임은
     통째로 사라진다 — calibrate 가 보였던 건 스윕이 몇 초간 돌아
     CAL_START 프레임이 오래 살아남았기 때문(운이 좋으면 CAL_OK 도 보임).
     수정: Runner 가 최근 이벤트 링버퍼(EVENT_KEEP개)를 유지해 '매 프레임'
     frame["events"] 로 다시 실어 보낸다. watcher 는 이벤트 JSON 전체를
     키로 dedup 하므로(_seen_event_keys) 중복 기록은 원래 안 생긴다.
     → read_rgb 포함 모든 단발 이벤트가 이벤트창에 반드시 도착한다.

  I) 색상 가이드 캘리브레이션(cal_color 위저드, 사용자 요구 — "무슨 색부터
     캘리브레이션해야 하는지, 그 색으로 캘리브레이션이 됐는지 확인까지"):
     대시보드 [cal_color] 를 누를 때마다 정해진 순서
       WHITE(흰 바닥) → BLACK(라인) → RED → GREEN → BLUE
     의 '다음 색' 하나를 샘플링한다(정지 상태 7회 중앙값).
       - 샘플이 그 색답지 않으면(지배비/밝기 검사) CAL_COLOR_FAIL 로
         이유를 알려주고 같은 색을 다시 기다린다(로봇은 움직이지 않는다).
       - 통과하면 CAL_COLOR_OK + 다음에 올릴 색을 CAL_COLOR_NEXT 로 안내.
       - 5색이 모두 모이면 rgb_sum_white/rgb_black_max/rgb_red·green·
         blue_ratio 를 실측에서 자동 산출(경계 = 목표색 지배비와 나머지
         색 최대 지배비의 중간, derive_rgb_params 순수 함수)해 set+save
         하고, 저장된 파라미터로 5색 샘플을 전부 재분류해 색마다
         CAL_COLOR_VERIFY ok/NG 를 띄운다. 하나라도 NG 면 CAL_COLOR_DONE
         에 어떤 색이 왜 안 갈리는지(separation) 남는다.
     [cal_color_restart] 는 WHITE 부터 다시 시작. 위저드 진행 상태는 세션
     reset 과 무관하게 유지된다(도중 [r] 눌러도 샘플이 안 날아간다).
     read_rgb 도 개선: 1회가 아니라 5회 중앙값 + 분류 이름을 같이 로그.
     rgb_* 파라미터의 max_step 은 위저드가 실측치를 직접 기록할 수 있게
     cal_* 와 같은 '제한 없음'(전체 범위)으로 올렸다.

실행(브릭):   python3 stages/ffrun1.py
문법 점검(PC): python3 -m py_compile stages/ffrun1.py lib/*.py
단위 테스트:  python3 tests/test_ffrun1_wizard.py (위저드/링버퍼, ev3dev2 불필요)
              미로/판단층은 final_run11 과 동일(test_final_run11_maze.py 커버)

===== 이하 final_run11 원문 헤더 =====
...
COL_NONE = 0
COL_BLACK = 1
COL_BLUE = 2                # §J — 노랑(4) 대신 파랑(ev3dev COLOR_BLUE 관례 코드)
COL_GREEN = 3
COL_RED = 5
COL_WHITE = 6

MARKER_COLORS = (COL_RED, COL_BLUE, COL_GREEN)
MARKER_NAMES = {COL_RED: "red", COL_BLUE: "blue", COL_GREEN: "green"}
# 로그 표기용 전체 색 이름(§I) — read_rgb/cal_color 이벤트에서 코드 대신 이름.
COLOR_LOG_NAMES = {COL_NONE: "none", COL_BLACK: "black", COL_GREEN: "green",
                   COL_BLUE: "blue", COL_RED: "red", COL_WHITE: "white"}
...
START_EXIT_MM = 50          # 출발 파랑에서 벗어나는 거리(§J)
...
# 단발 이벤트 링버퍼(§H) — 최근 이벤트를 매 프레임 다시 실어 watcher 폴링
# (4Hz)이 프레임을 놓쳐도 이벤트가 유실되지 않게 한다. watcher 가 dedup.
EVENT_KEEP = 30

# 색상 가이드 캘리브레이션(§I). 순서 고정 — black 밝기 검사가 white 실측을
# 필요로 하므로 white 가 항상 먼저다.
CAL_COLOR_ORDER = ("white", "black", "red", "green", "blue")
CAL_COLOR_EXPECT = {"white": COL_WHITE, "black": COL_BLACK, "red": COL_RED,
                    "green": COL_GREEN, "blue": COL_BLUE}
CAL_COLOR_SAMPLES = 7       # 정지 상태 재판독 횟수(채널별 중앙값 사용)
CAL_COLOR_GAP_S = 0.02
CAL_COLOR_MIN_DOM = 1.15    # 색 샘플 합격 최소 지배비(r/g 등) — 이보다 밋밋하면 그 색이 아니다
CAL_COLOR_SEP = 1.10        # 목표색 지배비가 나머지 색 최대의 이 배수는 넘어야 경계를 그을 수 있다
CAL_COLOR_WHITE_MIN_SUM = 150.0   # 흰 바닥 r+g+b 최소 — 검정(전형 ~100 안팎)을 white 로 오인 방지
CAL_COLOR_WHITE_MAX_DOM = 1.8     # white 의 빨강/초록/파랑 지배비 상한 — 실기 흰 바닥은 g 채널이 강해
                                  # g/r 1.2~1.3 까지 정상(진짜 초록은 3 이상). MIN_DOM(1.15)보다 느슨해야 한다.
                                  # 파랑도 같은 상한 — 흰 바닥의 blue 지배비 min(b/r, g/r)≈0.79,
                                  # 진짜 파랑은 3 안팎(§J 실측)이라 여유가 크다.
CAL_COLOR_BLACK_MAX_BRIGHT = 55.0  # 검정 샘플 밝기(% of white) 상한 — 넘으면 검정이 아니다
READ_RGB_SAMPLES = 5        # read_rgb 액션 중앙값 샘플 수(§I)
...
# cal_* 는 calibrate 액션이 실측치를 직접 기록하므로 max_step=100(제한 없음).
# cal_l/r 기본값 black 0 / white 100 = 정규화 항등(미보정 시 v13 동작).
# cal_c_* 기본값은 §K 정지 실측치(스윕보다 정확) — 브릭 저장값이 있으면 그쪽이 이긴다.
PARAM_TABLE = (
    ...
    # §K 실측: 엣지 정위치 norm_c ≈ 64(검/백 산술 중간 51보다 밝다).
    ("edge_target",     64,    20,  80,   10,   5,    "%"),
    ...
    # 중앙 RGB 밝기 기준 — §K 실측(2026-07-10): black 11.5 / white 89.8~91.7.
    ("cal_c_black",     12,    0,   100,  100,  1,    "%"),
    ("cal_c_white",     91,    0,   100,  100,  1,    "%"),
    # RGB 색판정(classify_rgb) — cal_color 위저드(§I)가 실측치를 직접 기록
    # 하므로 cal_* 처럼 max_step 을 전체 범위로 둔다(수동 미세조정도 가능).
    # rgb_sum_white: 흰 바닥 위 r+g+b 합(밝기 100% 기준).
    ("rgb_sum_white",   600,   100, 1100, 1000, 10,   ""),
    ("rgb_black_max",   25,    0,   100,  100,  1,    "%"),   # 밝기 이 미만 = 검정
    ("rgb_red_ratio",   1.8,   1.0, 4.0,  3.0,  0.1,  "x"),   # r > x*g, r > x*b
    ("rgb_green_ratio", 1.25,  1.0, 4.0,  3.0,  0.05, "x"),   # g > x*r, g > x*b
    # 파랑(§J)은 'r 최약' 축 — 이 센서는 파랑을 청록처럼 읽는다(g>b).
    # 실측: 파랑 min(b/r,g/r)≈2.98 vs 흰 바닥 0.79. cal_color 실측으로 조정.
    ("rgb_blue_ratio",  1.5,   1.0, 6.0,  5.0,  0.1,  "x"),   # b > x*r, g > x*r
...
SAVE_PATH = os.path.join(_ROOT, "config", "ffrun1.json")
STAGE_NAME = "ffrun1"

# [1]/[2] 키는 final_run11 과 동일하게 유지(손버릇 보호), 새 위저드는 [3]/[4].
ACTIONS = [
    {"name": "calibrate", "label": "Calibrate L/C/R on line (sweep)"},
    {"name": "read_rgb", "label": "Read Center RGB (+classify)"},
    {"name": "cal_color", "label": "Cal Color wizard: sample NEXT color"},
    {"name": "cal_color_restart", "label": "Cal Color wizard: restart (WHITE)"},
...
def classify_rgb(r, g, b, snap):
    ...
    색은 채널 지배 비율로 판정(개체차에 강하도록 절대값 대신 비율):
      ...
      파랑: b,g 둘 다 r 의 rgb_blue_ratio 배 초과(§J — 센서가 파랑을
            청록으로 읽어 g>b, 판정축은 'r 최약')
      초록: g 가 r,b 둘 다 rgb_green_ratio 배 초과
      흰색: 밝기 >= 60 / 그 외: 없음(경계·회색)
    파랑을 초록보다 먼저 검사한다 — 파랑도 g/r 가 커서(실측 3.65) 초록
    판정과 축을 공유하는데, 진짜 초록은 b 가 최약(실측 b/r 0.3)이라 파랑
    검사를 절대 통과 못 하므로 이 순서가 안전하다(§J).
    """
    ...
    blue_x = snap["rgb_blue_ratio"]
    if bb > blue_x * rr and gg > blue_x * rr:
        return COL_BLUE, bright
...
# ---- 색상 가이드 캘리브레이션 순수 함수(§I) — PC 단독 테스트 가능 ----

def median3(values):
    """정렬 중앙값(홀수 권장). 정지 상태 재판독의 1프레임 튐을 거른다."""
    ordered = sorted(values)
    return ordered[len(ordered) // 2]


def _ratio(a, b):
    """채널비 a/b — 0 나눗셈 가드(분모 최소 1)."""
    return float(a) / max(float(b), 1.0)


def color_dominance(name, rgb):
    """색 'name' 답기 위한 지배비 — classify_rgb 의 판정식과 같은 축.

    red: min(r/g, r/b) / green: min(g/r, g/b) / blue: min(b/r, g/r)(§J —
    센서가 파랑을 청록으로 읽어 g>b 이므로 'r 최약' 축, 실측 파랑 2.98).
    이 값이 rgb_*_ratio 보다 크면 classify_rgb 가 그 색으로 판정한다.
    """
    r, g, b = rgb
    if name == "red":
        return min(_ratio(r, g), _ratio(r, b))
    if name == "green":
        return min(_ratio(g, r), _ratio(g, b))
    if name == "blue":
        return min(_ratio(b, r), _ratio(g, r))
    raise ValueError("no dominance for color: {}".format(name))


def check_color_sample(name, rgb, white_rgb):
    """위저드 샘플이 요구한 색답게 읽혔는지 검사(순수). (ok, 사유, detail).

    사용자가 안내와 다른 색 위에 올렸거나(예: red 차례에 green 위) 센서가
    스티커를 반쯤 벗어난 경우를 잡는다. white 는 절대 밝기, black 은 white
    대비 밝기, 유채색은 지배비(CAL_COLOR_MIN_DOM)로 판정한다.
    """
    r, g, b = rgb
    total = float(r) + float(g) + float(b)
    detail = {"r": r, "g": g, "b": b, "sum": int(round(total))}
    if name == "white":
        if total < CAL_COLOR_WHITE_MIN_SUM:
            return False, "WHITE_TOO_DARK", detail
        # 밝기만으론 유채색 스티커도 통과한다 — 색이 강하게 서 있으면 흰 바닥이
        # 아니다. 실기 흰 바닥도 g 채널이 20~30% 강하므로 상한은 넉넉히(1.8).
        if color_dominance("red", rgb) >= CAL_COLOR_WHITE_MAX_DOM:
            return False, "LOOKS_RED", detail
        if color_dominance("green", rgb) >= CAL_COLOR_WHITE_MAX_DOM:
            return False, "LOOKS_GREEN", detail
        if color_dominance("blue", rgb) >= CAL_COLOR_WHITE_MAX_DOM:
            return False, "LOOKS_BLUE", detail
        return True, "OK", detail
    white_sum = max(float(white_rgb[0] + white_rgb[1] + white_rgb[2]), 1.0)
    bright = 100.0 * total / white_sum
    detail["bright"] = round(bright, 1)
    if name == "black":
        if bright > CAL_COLOR_BLACK_MAX_BRIGHT:
            return False, "BLACK_TOO_BRIGHT", detail
        return True, "OK", detail
    dom = color_dominance(name, rgb)
    detail["dom"] = round(dom, 2)
    if dom < CAL_COLOR_MIN_DOM:
        return False, "NOT_{}_ENOUGH".format(name.upper()), detail
    # (§J) 파랑은 청록으로 읽혀 g/r 축을 초록과 공유한다 — green 차례에
    # 파랑 스티커를 대면 green 지배비 min(g/r, g/b)가 1.2 안팎으로 MIN_DOM
    # 을 아슬하게 넘을 수 있어 blue 지배비로 교차 거부한다(진짜 초록의
    # blue 지배비는 0.3 안팎). 그 외 오배치는 NOT_*_ENOUGH 로 걸린다.
    if (name == "green" and
            color_dominance("blue", rgb) >= CAL_COLOR_WHITE_MAX_DOM):
        return False, "LOOKS_BLUE", detail
    return True, "OK", detail


def derive_rgb_params(samples):
    """5색 실측(samples: 이름→(r,g,b)) → rgb_* 파라미터 산출(순수).

    각 경계는 '목표색의 지배비'와 '나머지 색들 지배비 최대'의 중간에 긋는다
    (rgb_black_max 는 검정 밝기와 나머지 최저 밝기의 중간). 목표가 나머지의
    CAL_COLOR_SEP 배를 못 넘으면 그 색은 separation 실패로 보고한다 —
    경계는 그어 주되(중간값) verify 에서 NG 로 드러난다.

    반환: (params dict, report dict). report[색] = {"target":…, "others":…,
    "sep_ok": bool}. 파라미터는 PARAM_TABLE 한계 안으로 클램프해 params.set
    이 거부하지 않게 한다.
    """
    white_sum = max(float(sum(samples["white"])), 1.0)

    def bright(name):
        return 100.0 * float(sum(samples[name])) / white_sum

    report = {}
    params = {"rgb_sum_white": clamp(int(round(white_sum)), 100, 1100)}

    black_b = bright("black")
    floor_b = min(bright(n) for n in ("red", "green", "blue", "white"))
    params["rgb_black_max"] = round(clamp((black_b + floor_b) / 2.0,
                                          0.0, 100.0), 1)
    report["black"] = {"target": round(black_b, 1),
                       "others": round(floor_b, 1),
                       "sep_ok": floor_b >= black_b * CAL_COLOR_SEP}

    limits = {"red": (1.0, 4.0), "green": (1.0, 4.0), "blue": (1.0, 6.0)}
    for name in ("red", "green", "blue"):
        target = color_dominance(name, samples[name])
        others = max(color_dominance(name, samples[n])
                     for n in CAL_COLOR_ORDER if n != name)
        others = max(others, 1.0)
        lo, hi = limits[name]
        params["rgb_{}_ratio".format(name)] = round(
            clamp((target + others) / 2.0, lo + 0.05, hi), 2)
        report[name] = {"target": round(target, 2),
                        "others": round(others, 2),
                        "sep_ok": target >= others * CAL_COLOR_SEP}
...
def explorer_to_graph(nodes, arm_ends):
    ...
    mark: 리프ID → "red"|"green"|"blue"|"dead_end"|"home" (arm_ends 기록).
    """
    adj = {}
    mark = {}
    for nid in nodes:
...
class Explorer(object):
    ...
    def __init__(self):
        ...
        self.probe_arm = None
        self.goto_arm = None
        self.arm_ends = {}      # (노드id, 방위) -> "red"|"green"|"blue"|"dead_end"
    ...
    def start_home(self):
        ...
        전(TO_FIRST) 초록이면 지도가 없다 — 빈 계획(온 길 직진 = 파랑)으로
    ...
    def on_arrive_home(self, kind, has_left=False, has_right=False,
                       has_straight=False):
        """복귀 중 도착(junction/red/green/blue/dead_end) 처리 — (move, events).
...
class Runner(object):
    ...
    def __init__(self, hw, params, tele, log):
        ...
        self.ex = Explorer()
        self.pid = PidSteer()
        # 단발 이벤트 링버퍼(§H) — 매 프레임 재전송용. 세션 reset 과 무관.
        self._recent_events = []
        # 색상 위저드 진행 상태(§I) — 세션 reset 에도 유지(재시작은 전용 액션).
        self.cal_color_idx = 0
        self.cal_color_samples = {}
    ...
    def _init_session_state(self):
        ...
        self.return_elapsed = None   # 초록→파랑 소요(s)
        self.last_turn = 0.0
    ...
    def publish(self, mode, **extra):
        ...
        # §H: 새 이벤트를 링버퍼에 쌓고 '최근 이벤트 전체'를 매 프레임 실어
        # 보낸다. Telemetry 는 최신 프레임 1개만 보관하므로 이벤트를 한
        # 프레임에만 실으면 watcher(4Hz) 폴링 사이에 유실된다 — read_rgb 등
        # 단발 액션 이벤트가 대시보드에 안 뜨던 원인. 중복은 watcher 가
        # 이벤트 JSON 키로 dedup 한다.
        events = self.log.drain_events()
        if events:
            self._recent_events.extend(events)
            del self._recent_events[:-EVENT_KEEP]
        if self._recent_events:
            frame["events"] = list(self._recent_events)
    ...
    def handle_pending(self):
        ...
        try:
            if action == "calibrate":
                ...
                r, g, b = self._read_rgb_median(READ_RGB_SAMPLES)
                color, bright = classify_rgb(r, g, b, snap)
                norm_c = normalize(bright, snap["cal_c_black"], snap["cal_c_white"])
                self.log.log("COLOR_READ", "DO_TRIGGER", r=r, g=g, b=b,
                             color=color, name=COLOR_LOG_NAMES.get(color, str(color)),
                             bright=round(bright, 1), norm_c=round(norm_c, 1),
                             samples=READ_RGB_SAMPLES)
                self.publish("read_rgb", r=r, g=g, b=b, color=color,
                             bright=round(bright, 1), norm_c=round(norm_c, 1))
            elif action == "cal_color":
                self.cal_color_step()
            elif action == "cal_color_restart":
                self.cal_color_idx = 0
                self.cal_color_samples = {}
                self.log.log("CAL_COLOR", "RESTART",
                             next="WHITE", step="1/{}".format(len(CAL_COLOR_ORDER)),
                             msg="PUT_CENTER_ON_WHITE_THEN_PRESS_CAL_COLOR")
    ...
    def _read_rgb_median(self, n):
        """정지 상태에서 중앙 RGB 를 n 회 읽어 채널별 중앙값 (r,g,b) 반환."""
        rs = []
        gs = []
        bs = []
        for _i in range(n):
            ...
            gs.append(g)
            bs.append(b)
            time.sleep(CAL_COLOR_GAP_S)
        return median3(rs), median3(gs), median3(bs)

    def _cal_color_prompt(self):
        """지금 올려야 할 색 안내 이벤트(위저드 진행 상황 표시 공용)."""
        total = len(CAL_COLOR_ORDER)
        if self.cal_color_idx >= total:
            self.log.log("CAL_COLOR", "ALL_DONE",
                         msg="PRESS_CAL_COLOR_RESTART_TO_REDO")
            return
        name = CAL_COLOR_ORDER[self.cal_color_idx]
        self.log.log("CAL_COLOR_NEXT", "PLACE_AND_PRESS",
                     place=name.upper(),
                     step="{}/{}".format(self.cal_color_idx + 1, total),
                     msg="PUT_CENTER_ON_{}_THEN_PRESS_CAL_COLOR".format(
                         name.upper()))

    def cal_color_step(self):
        """[cal_color] 1회 = 순서상 '다음 색' 샘플링(로봇은 움직이지 않는다).

        샘플이 그 색답지 않으면 FAIL 사유를 알려주고 같은 색을 다시 기다린다.
        5색이 모두 모이면 _cal_color_finish 가 파라미터 산출·저장·검증까지 한다.
        """
        self.hw.stop()
        total = len(CAL_COLOR_ORDER)
        if self.cal_color_idx >= total:
            self._cal_color_prompt()    # ALL_DONE — restart 안내
            return
        name = CAL_COLOR_ORDER[self.cal_color_idx]
        self.publish("cal_color")
        rgb = self._read_rgb_median(CAL_COLOR_SAMPLES)
        ok, reason, detail = check_color_sample(
            name, rgb, self.cal_color_samples.get("white", (1, 1, 1)))
        if not ok:
            detail = dict(detail)
            detail["place"] = name.upper()
            detail["msg"] = "STILL_WAITING_{}_FIX_POSITION_AND_PRESS_AGAIN".format(
                name.upper())
            self.log.log("CAL_COLOR_FAIL", reason, **detail)
            return
        self.cal_color_samples[name] = rgb
        self.cal_color_idx += 1
        detail = dict(detail)
        detail["place"] = name.upper()
        detail["step"] = "{}/{}".format(self.cal_color_idx, total)
        self.log.log("CAL_COLOR_OK", "SAMPLED", **detail)
        if self.cal_color_idx < total:
            self._cal_color_prompt()
        else:
            self._cal_color_finish()

    def _cal_color_finish(self):
        """5색 완료: rgb_* 산출 → set+save → 저장값으로 5색 재분류 검증.

        검증(사용자 요구 — "그 색으로 캘리브레이션이 됐는지 확인"): 방금
        저장한 파라미터로 classify_rgb 를 다시 돌려 색마다 OK/NG 를 띄운다.
        NG 색은 report 의 separation(목표 지배비 vs 나머지 최대)으로 원인이
        보인다 — 조명을 바꾸거나 스티커 위 위치를 고쳐 restart 하면 된다.
        """
        params, report = derive_rgb_params(self.cal_color_samples)
        set_fail = None
        for pname in sorted(params):
            ok, msg = self.params.set(pname, params[pname])
            if not ok:
                set_fail = "{}: {}".format(pname, msg)
                break
        saved, save_msg = self.params.save() if set_fail is None else (False,
                                                                       set_fail)
        snap = self.params.snapshot()
        all_ok = set_fail is None
        for name in CAL_COLOR_ORDER:
            r, g, b = self.cal_color_samples[name]
            got, bright = classify_rgb(r, g, b, snap)
            got_name = COLOR_LOG_NAMES.get(got, str(got))
            # separation 실패는 저장 샘플이 우연히 맞게 갈려도 NG — 경계가
            # 칼날 위라(예: 지배비 1.51 vs 나머지 1.50) 실주행 노이즈에
            # 바로 무너진다. others_dom 이 target_dom 에 붙어 있으면 원인.
            verify_ok = (got == CAL_COLOR_EXPECT[name] and
                         report.get(name, {}).get("sep_ok", True))
            all_ok = all_ok and verify_ok
            rep = report.get(name, {})
            self.log.log("CAL_COLOR_VERIFY",
                         "OK" if verify_ok else "NG",
                         place=name.upper(), classified=got_name,
                         bright=round(bright, 1),
                         target_dom=rep.get("target", "-"),
                         others_dom=rep.get("others", "-"),
                         sep_ok=rep.get("sep_ok", True))
        self.log.log("CAL_COLOR_DONE",
                     "ALL_VERIFIED" if all_ok else "CHECK_NG_COLORS",
                     saved=saved, save_msg=save_msg,
                     rgb_sum_white=params["rgb_sum_white"],
                     rgb_black_max=params["rgb_black_max"],
                     red_x=params["rgb_red_ratio"],
                     green_x=params["rgb_green_ratio"],
                     blue_x=params["rgb_blue_ratio"],
                     msg="DONE" if all_ok
                     else "NG_COLOR_RETRY_WITH_CAL_COLOR_RESTART")
    ...
    def handle_marker(self, color, context):
        """빨강/파랑/초록 마커 처리(정지 → 색 확정 → 미션 분기). 처리했으면 True.

        파랑@복귀 = 완주(그립 해제) / 초록@탐색 = 드롭·재파지 후 유턴·복귀계획 /
        복귀 중 마커 = 계획 스텝 소비 / 그 외(탐색 중 빨강·파랑) = 유턴."""
        ...
        if color == COL_BLUE and self.ex.mode == "HOME":
        ...
        # 탐색 중 빨강 = 경유지(명세 5): "red N" 후 유턴. 파랑 = 막다른 표식.
        if color == COL_RED:
            self.visits += 1
            self.say_red()
        self.turn("U")
    ...
    def _release_at_home(self):
        """파란점 완주: 물체를 내려놓는다(그립 해제)."""
        snap = self.params.snapshot()
        if self.grabbed:
            self.hw.grip_open(snap["grip_speed"], GRIP_SEC)
            self.grabbed = False
            self.log.log("DROP_HOME", "COLOR_BLUE")
    ...
    def wait_for_start(self):
        ...
        대기 중에도 handle_pending 이 돌므로 이 상태에서 calibrate / cal_color
        위저드 / read_rgb 를 먼저 실행하면 된다(로봇을 라인/스티커 위에 두고
        실행 — cal_color 는 CAL_COLOR_NEXT 이벤트가 안내하는 색 순서대로)."""
        snap = self.params.snapshot()
        ...
        self.last_marker_t = time.monotonic()   # 출발 파랑 재감지 방지
        return "go"
    ...
    def idle_after_done(self):
        """완주 후 대기 — 그 자리 파랑에서 무한 재출발하지 않도록 reset 을 기다린다."""
        self.hw.stop()
        while True:
            if self.stop_on:
                self.log.log("EMERGENCY_STOP", "NETWORK", source=self.stop_source)
    ...
    def new_session(self):
        ...
        if self.session == 1:
            ...
            self.log.log("SESSION_RESET", "DASHBOARD", source=self.reset_source,
                         session=self.session)
        # 출발 대기에 들어갈 때마다 위저드 진행 안내(§I) — 무슨 색부터
        # 캘리브레이션해야 하는지 이벤트창에서 바로 보인다.
        self._cal_color_prompt()
...
def migrate_saved_params(path):
    """§J — 노랑→파랑 이행: 저장 파일의 rgb_yellow_ratio 를 rgb_blue_ratio 로
    교체한다(값은 축이 달라 이월 불가 — 기본값, cal_color 로 재캘리브레이션).

    SharedParams._validate_all 은 알 수 없는 키/빠진 키가 하나라도 있으면
    파일 '전체'를 거부한다 — 마이그레이션 없이는 실기에서 애써 잡은 cal_*
    라인 캘리브레이션·임계값 튜닝까지 통째로 기본값으로 돌아가 버린다.
    """
    try:
        ...
        return False
    if not isinstance(saved, dict) or "rgb_yellow_ratio" not in saved:
        return False
    saved.pop("rgb_yellow_ratio", None)
    saved.setdefault("rgb_blue_ratio", INITIAL_PARAMS["rgb_blue_ratio"])
    try:
        with open(path, "w") as fp:
            json.dump(saved, fp, indent=2, sort_keys=True)
    except Exception:
        return False
    return True
...
def run():
    ...
    if migrate_saved_params(SAVE_PATH):
        print("ffrun1: migrated saved params (rgb_yellow_ratio -> "
              "rgb_blue_ratio, run cal_color to calibrate BLUE)")
    ...
    print("ffrun1 ready. 1) dashboard 'calibrate' on the line, 2) 'cal_color' "
          "wizard: WHITE -> BLACK -> RED -> GREEN -> BLUE (follow CAL_COLOR_NEXT "
          "events; each press samples one color and verifies it), 3) press CENTER "
          "button to start (random number + voice). grips the object when the "
          "ultrasonic sees one. dashboard 'reset' ([r]) restarts. "
    ...
    try:
        ...
        try:
            ...
            server.stop()
    print("ffrun1 stopped. sessions={}".format(runner.session))
