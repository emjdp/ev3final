#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""fxck1 — gg5 + 의심지점 원격 판단(주행/미션/UI 는 gg5 그대로).
...
원격 판단층(fxck1 신설 — 거의 원격 조종 중심):
  ...
  - 파라미터 승계: config/fxck1.json 이 없으면 gg5.json 의 실기 튜닝값을
    전부 자동 승계한다(속도 포함 — 주행층이 같다).
...
실행(브릭):   python3 stages/fxck1.py
문법 점검(PC): python3 -m py_compile stages/fxck1.py lib/*.py
단위 테스트:  python3 tests/test_fxck1_logic.py (ev3dev2 불필요)
"""
...
ACCEL_START_SPEED = 12      # 출발 가속 램프 시작 속도(%) — SLOW_SPEED 와 동급
...
SAVE_PATH = os.path.join(_ROOT, "config", "fxck1.json")
STAGE_NAME = "fxck1"

# gg5 실기 튜닝값 승계(최초 1회) — fxck1.json 이 없으면 gg5.json 에서
# 전부 가져온다(주행층이 같아 속도 계열도 그대로 승계).
GG5_SAVE_PATH = os.path.join(_ROOT, "config", "gg5.json")
SEED_SKIP = ()
        ...
        while not self.done:
            ...
            if near_node:
                ...
                if self.guard_since is not None:
                    # 가드(감속 직진)에서 순항으로 복귀 — 12%→순항도 램프로(gg5).
                    self.ramp_t0 = now
...
def seed_initial_from_gg5(initial):
    """fxck1 첫 실행 승계: fxck1.json 이 없으면 gg5.json 의 실기 튜닝값을
    initial(INITIAL_PARAMS 사본)에 병합해 반환한다(승계된 이름 리스트).

    SharedParams.set 은 max_step 제한이 있어 생성 '전'의 기본값 단계에서
    병합한다. 주행층이 gg5 와 같으므로 SEED_SKIP 은 비어 있고(속도도 승계),
    값은 fxck1 한계로 클램프한다. fxck1.json 이 이미 있으면 아무 것도 하지
    않는다(load_saved_into_defaults 가 저장값으로 덮는다)."""
    if os.path.exists(SAVE_PATH) or not os.path.exists(GG5_SAVE_PATH):
        return []
    try:
        with open(GG5_SAVE_PATH, "r") as fp:
            saved = json.load(fp)
    except Exception:
        return []
...
def run():
    ...
    seeded = seed_initial_from_gg5(initial)
    ...
    if seeded:
        log.log("PARAM_SEED", "GG5_CARRYOVER", names=",".join(seeded))
    print("fxck1 ready. 1) dashboard 'calibrate' on the line, 2) place robot "
          "on YELLOW and press the CENTER button to start. it line-follows "
          "like gg5 but STOPS at every junction and waits for dashboard keys: "
          "[a] left  [d] right  [w] u-turn  [s] back up  [x] straight. "
    ...
    try:
        ...
        try:
            ...
            server.stop()
    print("fxck1 stopped. sessions={}".format(runner.session))
