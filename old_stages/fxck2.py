#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""fxck2 — fxck1 + 조종 오버라이드(ijkl) + 의심지점 상대 감속.

조종 오버라이드층(fxck2 신설):
  - 주행 중 언제든 대시보드에서 i/j/k/l 을 누르면 자동 통제(라인추종·
    노드판정·복귀계획)를 무시하고 그 키의 이동 하나를 수행한 뒤 곧바로
    통제권을 자동 반환한다 — 사람이 잡고 있는 모드가 아니라 1회 개입.
      [i] 전진(OVERRIDE_STEP_MM)      [k] 후진(OVERRIDE_STEP_MM)
      [j] 좌로 30도(OVERRIDE_TURN_DEG) [l] 우로 30도 — 연타로 각도 누적
      [o] 통제권 반환 — 의심지점 대기를 끝내고 그대로 라인추종 재개
    반환 후엔 그 시점의 자동 상태로 돌아간다: 주행 중이면 그 자리에서
    라인추종 재개(가속 램프부터), 의심지점 대기(manual_wait) 중이면
    계속 대기([s] 후진과 같은 위치 교정 성격) — [o] 를 누르면 회전
    없이 대기를 끝내고 로봇에 통제권을 돌려준다(주행 중 [o] 는 이미
    로봇 통제이므로 무시). 연타하면 이동 중 눌린 키가 한 칸 큐에 남아
    라인추종 없이 이어서 수행된다(체이닝).
    회전은 30도 소각 피벗(turn_90_factor 로 스케일, coast→램프 스무스
    턴은 turn() 과 동일) — 90도 격자가 아니므로 heading 은 갱신하지
    않고 라인 재획득 스캔도 하지 않는다(시킨 것만 한다; 사람이 조종
    중이라 지도/heading 은 어차피 참고용). 출발 대기/완주 대기 중엔
    ijkl 을 무시한다(주행 중에만 무장 — override_armed). 대시보드 연동:
    i/j/k/l 은 내장 키와 겹치지 않아 일반 액션 폴스루로 아무 때나
    전송된다 — 대시보드 수정 없음. 이름이 manual_* 이라 [.] repeat/
    auto-rerun 재발사 제외도 그대로 받는다.
  - 의심지점 상대 감속: 노드 confirm 저속 직진·노드 전진(confirm_speed)
    과 노드 가드·SLOW_ON·유실 의심 감속(slow_speed)이 고정 7/12% 가
    아니라 base_speed 에 비례한다(base×0.25 / base×0.40, 하한 5/8%).
    기본 base 30% 에선 7.5/12% 로 fxck1 과 사실상 동일하고, base 를
    올리든 내리든 의심지점에선 '베이스 대비 천천히'가 유지된다.
  - 파라미터 승계: config/fxck2.json 이 없으면 fxck1.json(없으면
    gg5.json)의 실기 튜닝값을 전부 자동 승계한다.

원격 판단층(fxck1 신설 — 거의 원격 조종 중심):
  - 라인트레이싱은 gg5 와 동일하게 자동으로 한다. 단, 의심지점(노드 확정
    후 출구가 2개 이상인 분기)에선 Explorer 가 판단하지 않고 정지한 채
    사람의 대시보드 키 입력을 기다린다(manual_wait 모드, 알림 tone 1회).
      [a] 좌회전   [d] 우회전   [w] 180도 회전(유턴)
      [s] 조금 후진(MANUAL_BACKUP_MM — 후진 후 계속 대기)
      [x] 직진(그대로 통과 — 십자/T 의 직진 선택용)
    a/d/w/x 는 회전(또는 통과) 후 라인추종을 재개하고, s 는 위치만 고쳐
    다시 대기한다. 대기 중에도 pause/stop/reset/calibrate 등 기존 네트워크
    처리는 전부 동작한다.
  - 대시보드 연동: ACTIONS 가 원하는 키(key)를 선언하고, 대시보드는 로봇이
    manual_wait 모드일 때만 그 키를 액션으로 보낸다 — 그 외 상황에선 기존
    의미 그대로다(a=auto-rerun, s=STOP 등). 로봇 쪽도 manual_wait 대기
    중이 아니면 manual_* 액션을 무시하므로 이중으로 안전하다.
  - 그 이외는 전부 gg5 유지: 출구 1개(강제 커브)는 자동 통과, 마커
    (빨강 유턴/초록 배달·복귀 전환/노랑 완주)·유실 복구·막다른길 유턴·
    파지·UI(스톱워치/red N/LCD)·고속 적응 주행층 모두 그대로.
    사람이 조종하므로 Explorer 지도/복귀 계획은 사실상 쓰이지 않지만
    heading 갱신과 초록→HOME 전환(노랑 완주 조건)은 그대로 동작한다.

고속 적응 주행층(gg5 — "빨라도 안정" 4종 세트):
  1) 출발 가속 램프: 정지→재출발마다(버튼 출발/회전/노드/마커/유실복구 후)
     라인추종 순항속도를 ACCEL_START_SPEED(12%)에서 목표까지 accel_ramp_ms
     동안 선형 가속한다. 정지 상태에서 30%를 즉시 꽂으면 바퀴 슬립+자세
     흐트러짐으로 첫 1초가 가장 불안정했던 것 대응. 직진 프리미티브
     (출발 이탈/배달 전·후진)는 하드웨어 램프(set_ramp)로 같은 효과.
  2) 커브 자동 감속: |조향 에러| EMA 에 비례해 순항속도를 깎는다
     (corner_gain, 하한 CORNER_MIN_SPEED). 직선에선 base_speed 풀스피드,
     커브에 들어서면 에러가 커지는 즉시 감속 — 속도를 올려도 커브에서
     라인을 놓치지 않는 핵심 장치다.
  3) 속도 비례 조향: PID turn 과 turn 한계를 (현재속도/SPEED_REF=20) 배율로
     스케일한다. kp/ki/deadband 는 20% 기준 실기 튜닝값 그대로 두고,
     속도가 오르면 조향 권한도 비례해 올라 20% 때와 같은 궤적 체감을
     유지한다(20% 주행에선 배율 1.0 = gg3 과 완전 동일).
  4) 반응 주기 단축: 제어 루프 sleep 15ms→10ms. 30% 순항에선 프레임당
     이동거리가 1.5배라 그만큼 샘플 간격을 좁혀야 노드/유실을 안 놓친다.
  기본 속도: base_speed 20→30%(최대 60), turn_speed 5→8%, 직진 15→20%.
  회전이 오버슛하면 turn_speed 를 먼저 내릴 것(factor 재튜닝보다 먼저).
...
주행층(실기 검증된 구성 유지):
  - 조향: error = 정규화(우반사광) - 정규화(좌반사광) PID. 센서별 black/white
    캘리브레이션(대시보드 calibrate, 소각 스윕)으로 개체차 제거, 작은 적분항이
    모터 편향을 학습해 정상상태 치우침 제거, P soft-deadband 로 미세 헌팅 제거.
...
실행(브릭):   python3 stages/fxck2.py
문법 점검(PC): python3 -m py_compile stages/fxck2.py lib/*.py
단위 테스트:  python3 tests/test_fxck2_logic.py (ev3dev2 불필요)
"""
...
# 고정 주행값(라이브 튜닝 대상 아님)
STRAIGHT_SPEED = 20         # 출발 이탈 / 배달 전·후진(gg5: 15→20, hw 램프로 출발 완화)
# 의심지점 감속(fxck2): confirm/slow 는 고정 %가 아니라 base_speed 비례 —
# confirm_speed()/slow_speed() 참조. 기본 base 30% 에선 7.5/12% 로 fxck1
# 과 사실상 동일하고, base 를 튜닝해도 '베이스 대비 천천히'가 유지된다.
...
LOOP_DELAY_S = 0.010        # gg5: 15ms→10ms — 30% 순항의 프레임당 이동거리 보상
...
# 고속 적응(gg5) — 헤더 주석의 4종 세트 상수.
SPEED_REF = 20.0            # 조향(kp/ki/deadband) 실기 튜닝 기준 속도(%)
STEER_SCALE_MIN = 0.75      # 속도 비례 조향 배율 하한(저속에서 과소조향 방지)
STEER_SCALE_MAX = 2.0       # 배율 상한(base 40% 이상은 조향 권한 고정)
ACCEL_START_SPEED = 12      # 출발 가속 램프 시작 속도(%) — slow_speed(base 30) 동급
CORNER_MIN_SPEED = 12       # 커브 자동 감속 하한(%) — 이 밑으론 깎지 않는다
ERR_EMA_ALPHA = 0.30        # 커브 감속용 |error| EMA 계수(프레임 노이즈 흡수)
...
# 원격 판단(fxck1) — 의심지점 대기/수동 이동.
MANUAL_BACKUP_MM = 40       # [s] 조금 후진 거리
MANUAL_POLL_S = 0.05        # manual_wait 대기 폴링 주기
# 대시보드 manual_* 액션 → turn() 이동. manual_back 은 이동이 아니라
# 후진 후 재대기라 여기 없다(manual_decide 참조).
MANUAL_MOVES = {"manual_left": "L", "manual_right": "R",
                "manual_uturn": "U", "manual_straight": "S"}

# 조종 오버라이드(fxck2) — ijkl 로 이동 1회 개입, 통제권은 자동 반환.
# 이름은 manual_* 접두를 유지한다: 대시보드 _run_action 이 last_action 으로
# 기록하지 않아 [.] repeat/auto-rerun 이 이동을 재발사하지 않는다.
OVERRIDE_STEP_MM = 50       # [i]/[k] 1회 전·후진 거리
OVERRIDE_TURN_DEG = 30      # [j]/[l] 1회 회전각(로봇 도) — 연타로 누적
OVERRIDE_MOVES = {"manual_spin_left": "L", "manual_spin_right": "R"}
OVERRIDE_CMDS = frozenset(("manual_fwd", "manual_rev", "manual_spin_left",
                           "manual_spin_right", "manual_resume"))
...
PARAM_TABLE = (
    # gg5: base 20→30(최대 60). 커브 자동 감속+속도 비례 조향이 받쳐준다.
    ("base_speed",      30,    5,   60,   5,    1,    "%"),
    ("kp",              0.17,  0.0, 3.0,  0.1,  0.01, ""),
    ("ki",              0.06,  0.0, 0.5,  0.05, 0.01, ""),
    ("deadband",        3.0,   0.0, 20.0, 2.0,  0.5,  ""),
    ("turn_speed",      8,     5,   40,   5,    1,    "%"),   # gg5: 5→8 — 회전 시간 단축
    # gg5 고속 적응 파라미터.
    ("accel_ramp_ms",   700,   0,   2000, 200,  50,   "ms"),  # 정지→재출발 가속 램프 시간
    ("corner_gain",     0.5,   0.0, 2.0,  0.25, 0.05, ""),    # |err| EMA 당 감속량(%/err)
...
SAVE_PATH = os.path.join(_ROOT, "config", "fxck2.json")
STAGE_NAME = "fxck2"

# 실기 튜닝값 승계(최초 1회) — fxck2.json 이 없으면 fxck1.json(없으면
# gg5.json)에서 전부 가져온다(주행층이 같아 속도 계열도 그대로 승계).
SEED_PATHS = (os.path.join(_ROOT, "config", "fxck1.json"),
              os.path.join(_ROOT, "config", "gg5.json"))
SEED_SKIP = ()

# manual_* 은 "key" 로 대시보드 키를 지정한다 — 대시보드는 로봇이
# manual_wait 모드일 때만 이 키를 액션으로 보낸다(그 외엔 기존 의미 유지).
ACTIONS = [
    ...
    {"name": "manual_left", "label": "Manual LEFT", "key": "a"},
    {"name": "manual_right", "label": "Manual RIGHT", "key": "d"},
    {"name": "manual_uturn", "label": "Manual U-TURN", "key": "w"},
    {"name": "manual_back", "label": "Manual BACK-UP", "key": "s"},
    {"name": "manual_straight", "label": "Manual STRAIGHT", "key": "x"},
    # 조종 오버라이드(fxck2) — 이동 1회 개입, 통제권 자동 반환.
    {"name": "manual_fwd", "label": "Override FORWARD", "key": "i"},
    {"name": "manual_rev", "label": "Override BACKWARD", "key": "k"},
    {"name": "manual_spin_left", "label": "Override LEFT", "key": "j"},
    {"name": "manual_spin_right", "label": "Override RIGHT", "key": "l"},
    {"name": "manual_resume", "label": "Release to auto (end wait)",
     "key": "o"},
]
...
def adaptive_base(target, corner_gain, err_ema, ramp_ms, elapsed_ms):
    ...
    1) 커브 감속: target 에서 corner_gain × err_ema 만큼 깎되
       CORNER_MIN_SPEED(target 이 그보다 작으면 target) 밑으론 안 내려간다.
       err_ema 는 |조향 에러|의 EMA — 직선에선 0 근처라 풀스피드,
       커브에선 에러가 커지는 즉시 비례 감속된다.
    2) 가속 램프: 재출발 후 elapsed_ms < ramp_ms 동안은
       ACCEL_START_SPEED→감속 반영 속도로 선형 가속(캡). 정지 직후
       고속을 즉시 꽂을 때의 슬립/자세 흐트러짐을 없앤다.
    """
...
def confirm_speed(base_speed):
    """의심지점 confirm 저속 직진/노드 전진 속도(순수, fxck2).

    고정 7% 가 아니라 base_speed 비례(×CONFIRM_SPEED_FRAC, 하한 MIN) —
    base 를 어떻게 튜닝해도 의심지점에선 베이스 대비 같은 비율로 느리다."""
    return max(float(CONFIRM_SPEED_MIN),
               float(base_speed) * CONFIRM_SPEED_FRAC)


def slow_speed(base_speed):
    """감속 주행(SLOW_ON/유실 의심/노드 가드) 속도(순수, fxck2).

    고정 12% 가 아니라 base_speed 비례(×SLOW_SPEED_FRAC, 하한 MIN)."""
...
class PidSteer(object):
    ...
    D: KD 고정 + raw derivative clamp + EMA.

    속도 비례 조향(gg5): turn 과 turn 한계를 base_speed/SPEED_REF 배율
    (STEER_SCALE_MIN~MAX 클램프)로 스케일한다. 게인은 SPEED_REF(20%)에서
    튜닝된 값 그대로 — 20% 주행이면 배율 1.0 이라 gg3 과 완전 동일하고,
    30% 순항에선 1.5배 조향 권한으로 같은 궤적 체감을 유지한다.
    """
...
class Runner(object):
    ...
    def __init__(self, hw, params, tele, log):
        ...
        # 원격 판단(fxck1) — manual_decide 대기 중에만 manual_cmd 를 받는다.
        self.manual_waiting = False
        self.manual_cmd = None
        # 조종 오버라이드(fxck2) — 주행 중(override_armed)에만 받는다.
    ...
    def on_do(self, action, args):
        if action == "reset":
            ...
            return {"queued": "reset"}
        if action in OVERRIDE_CMDS:
            # 조종 오버라이드(fxck2) — 주행(탐색/복귀/의심지점 대기) 중이면
            # 언제든 통제권을 뺏는다. 출발 대기/완주 대기에선 무시.
            with self._pending_lock:
                if not self.override_armed:
                    return {"ignored": action, "reason": "not_driving"}
                self.override_cmd = action
            return {"queued": action}
        if action.startswith("manual_"):
            # 의심지점 대기 중에만 유효 — 주행 중 잘못 눌린 키는 무시한다
            # (대시보드도 manual_wait 모드에서만 보내지만 이중 안전장치).
            with self._pending_lock:
                if not self.manual_waiting:
                    return {"ignored": action, "reason": "not_in_manual_wait"}
    ...
    def reset_steer(self):
        ...
        # 모션 프리미티브(회전/노드/복구/파지) 후 = 정지→재출발 지점이므로
        # 가속 램프를 새로 시작한다(gg5). 커브 감속 EMA 도 새 구간에서 재학습.
        self.err_ema = 0.0
    ...
    def straight(self, dist_mm, speed, mode="advancing"):
        ...
        gg5: 정지 상태에서 시작하는 프리미티브라 하드웨어 가속 램프
        (STRAIGHT_RAMP_MS)를 걸어 출발을 점진 가속한다(감속=0, 크리스프 정지).
        회전(turn)의 set_ramp 와 같은 방식 — finally 에서 0 복원."""
        self.hw.reset_encoders()
        if dist_mm <= 0:
            return 0.0
        ...
        try:
            ...
            self.hw.stop()
            self.hw.set_ramp(0)     # 라인추종 조향엔 램프 미적용(gg5)
    ...
    def handle_marker(self, color, context):
        ...
        if color is None:
            ...
            # 확정 판독을 위해 정지했으므로 재개는 가속 램프부터(gg5).
            self.ramp_t0 = time.monotonic()
    ...
    def confirm_node(self, first_bits, snap):
        ...
        self.reset_steer()
        self.hw.reset_encoders()
        creep = confirm_speed(snap["base_speed"])   # base 비례 감속(fxck2)
    ...
    def handle_node(self, bits, creep_mm):
        ...
        # 의심지점(출구 2개 이상) — Explorer 대신 사람이 판단한다(fxck1).
        self.manual_decide(has_left, has_right, has_straight)

    def manual_decide(self, has_left, has_right, has_straight):
        """의심지점 원격 판단(fxck1): 정지한 채 대시보드 키를 기다린다.

        [a] 좌회전 / [d] 우회전 / [w] 유턴 / [x] 직진 통과 /
        [s] 조금 후진(MANUAL_BACKUP_MM) 후 계속 대기.
        회전은 gg5 turn() 그대로라 heading 갱신·PID 리셋·회전 후 라인
        재획득까지 동일하다. 대기 중에도 stop/reset/pause/calibrate 등
        기존 네트워크 처리가 전부 동작한다(manual_wait 모드로 publish).
        ijkl(조종 오버라이드)이 오면 그 이동 하나만 수행하고 계속
        대기한다 — [s] 후진과 같은 위치 교정 성격. [o] 는 회전 없이
        대기를 끝내고 통제권을 로봇에 돌려준다(라인추종 재개, fxck2)."""
        self.hw.stop()
        self.log.log("MANUAL_WAIT", "SUSPECT_POINT",
                     has_left=has_left, has_right=has_right,
                     has_straight=has_straight)
        with self._pending_lock:
            self.manual_cmd = None      # 대기 전 stale 입력 제거
        ...
        try:
            while True:
                ...
                with self._pending_lock:
                    cmd = self.manual_cmd
                    self.manual_cmd = None
                    ocmd = self.override_cmd
                    self.override_cmd = None
                if ocmd == "manual_resume":
                    # [o](fxck2) — 사람이 손을 뗀다: 회전 없이 대기를
                    # 끝내고 통제권을 로봇에 돌려준다(라인추종 재개).
                    self.log.log("MANUAL_MOVE", "RESUME_AUTO")
                    self.reset_steer()
                    return
                if ocmd is not None:
                    # ijkl(fxck2) — 이동 1회 수행 후 계속 대기(위치 교정,
                    # [s] 후진과 같은 성격 — 통제권은 자동 반환된다).
                    self.manual_override(ocmd)
                    continue
                if cmd is None:
                    # 정지 대기 상태라 pause 는 별도 처리 없이 그대로 대기.
                    self.publish("manual_wait", has_left=has_left,
                                 has_right=has_right,
                                 has_straight=has_straight)
                    time.sleep(MANUAL_POLL_S)
                    continue
                if cmd == "manual_back":
                    self.log.log("MANUAL_MOVE", "BACKUP",
                                 backup_mm=MANUAL_BACKUP_MM)
                    self.straight(MANUAL_BACKUP_MM, -STRAIGHT_SPEED,
                                  mode="manual_backup")
                    continue                    # 위치만 고치고 계속 대기
                move = MANUAL_MOVES.get(cmd)
                if move is None:
                    continue
                self.log.log("MANUAL_MOVE", cmd.upper(), move=move)
                self.turn(move)                 # "S" 는 즉시 반환(직진 통과)
                self.reset_steer()              # 직진 통과도 램프부터 재출발
                return
        finally:
            with self._pending_lock:
                self.manual_waiting = False
    ...
    # ---- 조종 오버라이드(fxck2) — ijkl 1회 개입, 통제권 자동 반환 ----

    def manual_override(self, cmd):
        """오버라이드 이동 1회: 정지 → 시킨 이동 → 곧바로 통제권 자동 반환.

        [i] 전진 / [k] 후진(OVERRIDE_STEP_MM 씩) /
        [j] 좌로 / [l] 우로 OVERRIDE_TURN_DEG(30도)씩 — 연타로 누적.
        사람이 잡고 있는 모드가 없다 — 호출부(라인추종/의심지점 대기)가
        곧바로 재개된다. 이동 중 눌린 다음 키는 한 칸 큐(override_cmd)에
        남아 호출부 루프가 이어서 소비한다 — 연타하면 사이에 라인추종
        없이 체이닝된다(마지막 입력이 이긴다)."""
        self.hw.stop()
        if not self.interrupted():
            self._override_move(cmd)
        self.reset_steer()      # 반환 지점부터 가속 램프로 재출발

    def _override_move(self, cmd):
        """오버라이드 이동 1회 — 회전은 30도 세분 피벗(_override_pivot)."""
        move = OVERRIDE_MOVES.get(cmd)
        if move is not None:
            self._override_pivot(move)
            return
        if cmd == "manual_fwd":
            self.log.log("OVERRIDE_MOVE", "FORWARD", step_mm=OVERRIDE_STEP_MM)
            self.straight(OVERRIDE_STEP_MM, STRAIGHT_SPEED,
                          mode="override_move")
        elif cmd == "manual_rev":
            self.log.log("OVERRIDE_MOVE", "BACKWARD", step_mm=OVERRIDE_STEP_MM)
            self.straight(OVERRIDE_STEP_MM, -STRAIGHT_SPEED,
                          mode="override_move")

    def _override_pivot(self, move):
        """오버라이드 소각 피벗(L/R): OVERRIDE_TURN_DEG(로봇 30도)씩 회전.

        coast→가속 램프 스무스 턴은 turn() 과 동일하되, 90도 격자가
        아니므로 heading 은 갱신하지 않고(ex.apply_move 없음) 회전 후
        라인 재획득 스캔도 하지 않는다 — 시킨 것만 한다. 회전각은 90도
        피벗 실측치(BASE_PIVOT_DEG_90 × turn_90_factor)를 비례 스케일."""
        snap = self.params.snapshot()
        target = (BASE_PIVOT_DEG_90 * snap["turn_90_factor"]
                  * OVERRIDE_TURN_DEG / 90.0)
        left_dir, right_dir = (-1, 1) if move == "L" else (1, -1)
        speed = snap["turn_speed"]
        self.hw.coast()                 # 직전 brake-hold 해제 → 리셋 킥 방지
        self.hw.reset_encoders()
        ...
        try:
            ...
            while self.hw.enc_avg() < target:
                if self.interrupted():
                    break
                if self.paused:
                    self._hold_while_paused("override_turn")
                    if self.interrupted():
                        break
                    self.hw.drive_raw(left_dir * speed, right_dir * speed)
                self.publish("override_turn", target_deg=round(target, 1),
                             enc_avg=round(self.hw.enc_avg(), 1))
                time.sleep(0.005)
        finally:
            self.hw.stop()
            self.hw.set_ramp(0)         # 라인추종 조향엔 램프 미적용
        time.sleep(POST_TURN_SETTLE_S)
        self.log.log("OVERRIDE_TURN", "PIVOT_" + move,
                     deg=OVERRIDE_TURN_DEG, target_deg=round(target, 1),
                     enc_avg=round(self.hw.enc_avg(), 1))
    ...
    def wait_for_start(self):
        ...
        self.ramp_t0 = time.monotonic()         # 첫 라인추종도 가속 램프부터(gg5)
        return "go"

    def explore(self):
        """탐색+복귀 메인 루프. status(stop/reset/done) 반환.

        루프 동안만 조종 오버라이드(ijkl)를 무장한다(fxck2) — 출발 대기/
        완주 대기 중 눌린 키가 주행 시작 후 튀어나오지 않게 종료 시 큐도
        비운다."""
        with self._pending_lock:
    ...
    def _explore_loop(self):
        ...
        while not self.done:
            ...
            # 조종 오버라이드(fxck2): ijkl 이 눌렸으면 그 이동 하나만
            # 수행하고 곧바로 라인추종을 재개한다(통제권 자동 반환).
            with self._pending_lock:
                ocmd = self.override_cmd
                self.override_cmd = None
            if ocmd is not None and ocmd != "manual_resume":
                # [o] 는 주행 중엔 무시 — 이미 로봇 통제다(의심지점
                # 대기 해제 전용, manual_decide 참조).
                self.manual_override(ocmd)
                continue
            ...
            if near_node:
                ...
                if (now - self.guard_since) * 1000 < NODE_GUARD_MAX_MS:
                    guard = slow_speed(snap["base_speed"])  # base 비례(fxck2)
                ...
                if self.guard_since is not None:
                    # 가드(감속 직진)에서 순항으로 복귀 — 감속→순항도 램프로(gg5).
                    self.ramp_t0 = now
            ...
            if slow:
                base = slow_speed(snap["base_speed"])   # base 비례(fxck2)
            else:
                # gg5: 커브 자동 감속(직전 프레임까지의 |error| EMA) +
                # 재출발 가속 램프. 순수 함수 adaptive_base 참조.
            ...
            self.last_turn = turn
            # 다음 프레임의 커브 감속용 에러 EMA(1프레임 지연은 무해).
...
def seed_initial_from_prev(initial):
    """fxck2 첫 실행 승계: fxck2.json 이 없으면 SEED_PATHS(fxck1 → gg5) 중
    처음 존재하는 저장값을 initial(INITIAL_PARAMS 사본)에 병합해 반환한다
    (승계된 이름 리스트).

    SharedParams.set 은 max_step 제한이 있어 생성 '전'의 기본값 단계에서
    병합한다. 주행층이 같으므로 SEED_SKIP 은 비어 있고(속도도 승계),
    값은 fxck2 한계로 클램프한다. fxck2.json 이 이미 있으면 아무 것도 하지
    않는다(load_saved_into_defaults 가 저장값으로 덮는다)."""
...
def run():
    ...
    print("fxck2 ready. 1) dashboard 'calibrate' on the line, 2) place robot "
          "on YELLOW and press the CENTER button to start. junction keys "
          "([a] left [d] right [w] u-turn [s] back [x] straight) work like "
          "fxck1. NEW: press [i]/[k]/[j]/[l] anytime while driving to inject "
          "ONE manual move (fwd/back 50mm, left/right 30deg — tap repeatedly "
          "to add up) — control returns to the robot automatically after "
          "each move; [o] releases a waiting junction back to auto. "
          "suspect-point creep/slow speeds scale with base_speed. "
    ...
    try:
        ...
        try:
            ...
            server.stop()
    print("fxck2 stopped. sessions={}".format(runner.session))
