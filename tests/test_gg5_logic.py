#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""gg5(=gg3 판단층 + 고속 적응 주행층) 단위 테스트 — PC 단독 실행(ev3dev2 불필요).

gg5 신설 검증:
  - adaptive_base: 커브 자동 감속(에러 비례, 하한) + 출발 가속 램프(선형, 캡).
  - PidSteer 속도 비례 조향: 20%(SPEED_REF)에선 gg3 과 동일, 고속에선
    turn/한계가 배율만큼 커지고 상·하한 클램프가 걸린다.
...
실행: python3 tests/test_gg5_logic.py
"""
...
from stages.gg5 import (Explorer, PidSteer, node_bits, on_line, shortest_path,
                        turn_heading, adaptive_base, INITIAL_PARAMS,
                        HOME_CURVE_ASSUME_MAX, ACCEL_START_SPEED,
                        CORNER_MIN_SPEED, SPEED_REF, STEER_SCALE_MAX,
                        PID_TURN_LIMIT,
...
class AdaptiveBaseCase(unittest.TestCase):
    ...
    def test_ramp_caps_below_corner_slow(self):
        """램프 초반엔 커브 감속 결과보다도 낮은 캡이 이긴다."""
        self.assertEqual(adaptive_base(30, 0.5, 20.0, 700, 0),
                         float(ACCEL_START_SPEED))


class SpeedScaledSteerCase(unittest.TestCase):
    """gg5 속도 비례 조향 — SPEED_REF(20%)에선 gg3 동일, 고속에선 배율."""
    ...
    def test_ref_speed_unscaled(self):
        """배율 1.0 = kp×(error−deadband) 그대로(gg3 과 동일 동작)."""
